...
describe "using the server api" do
  context "with credentials present" do
    ...
    context "when I list all my servers (and I don't have any')" do
      When {VCR.use_cassette('server/show-servers') {run "rax show servers"}}
      Then {all_stdout =~ /you don't have any servers/}
      And {last_exit_status.should eql 0}
    end

    #context "when I create a server" do
    #  When {VCR.use_cassette('server/create-server') {run "rax create server"}}
    ...
    context "when I show a server" do
      When {VCR.use_cassette('server/show-server') {run "rax show server divine-reef"}}
      Then {last_exit_status.should eql 0}
    end
    context "when I destroy a server that exists" do
      When {VCR.use_cassette('server/destroy-server') {run "rax destroy server divine-reef"}}
